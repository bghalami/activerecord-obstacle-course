...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'finds order id of largest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    order_id = Order.all.order(id: :desc).first.id
    # ------------------------------------------------------------
  ...
  it 'returns the sorted names of items for a user order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      items_for_user_3_third_order = Order.find_by_id(9).items.pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
      orders = Order.joins(:order_items).where("order_items.item_id = 4 AND orders.user_id = 2")
    # -----------------------------------------------------------
  ...
  it 'returns items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
      ordered_items = Item.joins(:order_items).where("order_items.item_id = items.id").uniq
    # ---------------------------------------------------------------
  ...
  it 'returns the names of items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
      ordered_items_names = Item.joins(:order_items).where("order_items.item_id = items.id").pluck("items.name").uniq
    # ---------------------------------------------------------------
  ...
  it 'returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).select("users.name, count(user_id) as total_item_count").group(:name).order("users.name desc")
    # ---------------------------------------------------------------
  ...
  it 'returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = custom_results = User.joins(:order_items).select("name AS user_name, order_id, count(item_id) AS item_count").group(:order_id).order("users.name desc")
    # ---------------------------------------------------------------
